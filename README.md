<!-- 图片轮播展示墙开始 - GitHub兼容GIF动画方案 -->
<div align="center">

<img src="work_pic/carousel.gif" width="800" alt="XT-OPS智慧运维平台功能展示轮播" />
...
**为政企、军企、能源、金融等高度安全敏感行业量身打造的内网网络性能测速与终端资产管理平台**
...
| 流向 | 说明 |
|:----:|:-----|
| 浏览器 ← 内网服务器 | CSP `connect-src 'self'` 禁止任何外部请求 |
| 浏览器 → 内网服务器 | 所有请求仅限内网 |
...
| 角色 | 中文名 | 权限范围 | 条数 |
|:----:|:------:|:---------|:----:|
| `admin` | 系统管理员 | 全部权限（含系统配置、用户管理、IP映射、触发器） | 18 |
| `operator` | 操作员 | 日常操作（不含系统级配置） | 18 |
| `auditor` | 审计员 | 只读审计（仅查看+审计日志+数据导出） | 18 |
...
| 层级 | 组件 | 版本/端口 | 职责 |
|:----:|:-----|:----------|:-----|
| 接入层 | Nginx | :8009 (SSL) / :8080 (HTTP) | 反向代理 + CSP防护 + 静态资源 |
| 应用层 | PHP-FPM | 8.1 | 业务逻辑 + API处理 |
| 数据层 | MariaDB | 10.6 | 测速记录 + 终端信息 + 审计日志 + 配置 |

**前端资源：**

| 文件 | 说明 |
|:-----|:-----|
| `index.html` | 测速主页面（汽车仪表盘 + 110+随机特效） |
| `dashboard.html` | 管理大屏（7种布局 + 6种主题） |
| `xtops_panel_merged.js` | 大屏引擎（图表渲染 + 配置管理） |

**后端API：**

| 文件 | 说明 |
|:-----|:-----|
| `admin_api.php` | 管理API + 权限检查（requirePermission） |
| `dashboard_api.php` | 大屏数据查询 |
| `telemetry.php` | 测速数据入库 |
| `report_terminal.php` | 终端信息上报 |
| `xtops_collect_download.php` | 采集脚本生成（.bat/.sh） |

**终端：** PC / 手机 / 平板 → 浏览器访问 → Nginx → PHP-FPM → MariaDB

### 3.2 数据流架构

| 步骤 | 动作 | 流向 | 说明 |
|:----:|:-----|:-----|:-----|
| 1 | 访问测速页面 | 测速终端 → 内网服务器 | 打开HTTPS页面 |
| 2 | 下载采集脚本 | 内网服务器 → 测速终端 | .bat (Windows) / .sh (Linux) |
| 3 | 执行采集 | 测速终端本地 | 采集 IP + MAC + OS |
| 4 | 上报终端信息 | 测速终端 → 内网服务器 | POST 终端数据 |
| 5 | 入库 | 内网服务器 | 写入 `terminal_info` 表 |
| 6 | 执行测速 | 测速终端 → 内网服务器 | 下载/上传/延迟/抖动 |
| 7 | 入库 | 内网服务器 | 写入 `speedtest_users` 表 |
| 8 | 大屏实时展示 | 内网服务器 → 管理大屏 | 趋势/分布/统计图表 |
...
| 终端 | 分辨率 | 布局 | 说明 |
|:----:|:------:|:----:|:-----|
| 手机 | 375px | 单列 | 仪表盘自适应缩放，触摸操作优化 |
| 平板 | 768px | 双列 | 兼顾信息密度与可读性 |
| PC | 1920px | 多列 | 最大1120px内容宽度，双仪表盘并排 |
| 大屏TV | 4K/8K | 全展开 | 超宽屏支持，仪表盘铺满 |
...
| 价值维度 | 核心特性1 | 核心特性2 | 核心特性3 |
|:--------:|:----------|:----------|:----------|
| 合规价值 | 纯离线运行 | 等保2.0合规 | 数据不出网 |
| 安全价值 | 零外联设计 | CSP协议阻断 | 敏感文件保护 |
| 管理价值 | 三角色权限 | 审计全记录 | 操作可追溯 |
| 效率价值 | 一键部署 | 自动采集 | 实时大屏 |
| 成本价值 | 单镜像架构 | 无需买多套软件 | 零运维成本 |
| 体验价值 | 汽车仪表盘 | 110+特效 | 多端适配 |
